...
mod iadapter_mod {
    ...
    #[doc = "IAdapter was auto-generated with ethers-rs Abigen. More information at: https://github.com/gakonst/ethers-rs"]
    use std::sync::Arc;
    pub static IADAPTER_ABI: ethers::contract::Lazy<ethers::core::abi::Abi> =
    ...
    pub static IADAPTER_BYTECODE: ethers::contract::Lazy<ethers::core::types::Bytes> =
        ethers::contract::Lazy::new(|| "0x".parse().expect("invalid bytecode"));
    #[derive(Clone)]
    pub struct IAdapter<M>(ethers::contract::Contract<M>);
    impl<M> std::ops::Deref for IAdapter<M> {
        type Target = ethers::contract::Contract<M>;
        fn deref(&self) -> &Self::Target {
            &self.0
        }
    }
    impl<M: ethers::providers::Middleware> std::fmt::Debug for IAdapter<M> {
        fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
            f.debug_tuple(stringify!(IAdapter))
                .field(&self.address())
                .finish()
        }
    }
    impl<'a, M: ethers::providers::Middleware> IAdapter<M> {
        ...
        pub fn new<T: Into<ethers::core::types::Address>>(
            ...
            ethers::contract::Contract::new(address.into(), IADAPTER_ABI.clone(), client).into()
        }
    ...
    impl<M: ethers::providers::Middleware> From<ethers::contract::Contract<M>> for IAdapter<M> {
    ...
    impl ethers::core::abi::AbiEncode for IAdapterCalls {
        fn encode(self) -> Vec<u8> {
            match self {
                IAdapterCalls::GearboxAdapterType(element) => element.encode(),
                IAdapterCalls::GearboxAdapterVersion(element) => element.encode(),
                IAdapterCalls::CreditFacade(element) => element.encode(),
                IAdapterCalls::CreditManager(element) => element.encode(),
                IAdapterCalls::TargetContract(element) => element.encode(),
            }
        }
    }
    impl ::std::fmt::Display for IAdapterCalls {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                IAdapterCalls::GearboxAdapterType(element) => element.fmt(f),
                IAdapterCalls::GearboxAdapterVersion(element) => element.fmt(f),
                IAdapterCalls::CreditFacade(element) => element.fmt(f),
                IAdapterCalls::CreditManager(element) => element.fmt(f),
                IAdapterCalls::TargetContract(element) => element.fmt(f),
            }
        }
    }
    impl ::std::convert::From<GearboxAdapterTypeCall> for IAdapterCalls {
        fn from(var: GearboxAdapterTypeCall) -> Self {
            IAdapterCalls::GearboxAdapterType(var)
        }
    }
    impl ::std::convert::From<GearboxAdapterVersionCall> for IAdapterCalls {
        fn from(var: GearboxAdapterVersionCall) -> Self {
            IAdapterCalls::GearboxAdapterVersion(var)
        }
    }
    impl ::std::convert::From<CreditFacadeCall> for IAdapterCalls {
        fn from(var: CreditFacadeCall) -> Self {
            IAdapterCalls::CreditFacade(var)
        }
    }
    impl ::std::convert::From<CreditManagerCall> for IAdapterCalls {
        fn from(var: CreditManagerCall) -> Self {
            IAdapterCalls::CreditManager(var)
        }
    }
    impl ::std::convert::From<TargetContractCall> for IAdapterCalls {
        fn from(var: TargetContractCall) -> Self {
            IAdapterCalls::TargetContract(var)
        }
    }
}
